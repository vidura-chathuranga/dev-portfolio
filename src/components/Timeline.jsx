...
export const Timeline = ({ data }) => {
  ...
  return (
    <div className="c-space section-spacing" id="experience" ref={containerRef}>
      <h2 className="text-heading">Work Experiences</h2>
          ...
          >
            <div className="sticky z-40 flex flex-col items-center self-start max-w-xs md:flex-row top-40 lg:max-w-sm md:w-full">
              <div className="absolute flex items-center justify-center w-10 h-10 rounded-full -left-[15px] bg-midnight">
                ...
                <div className="flex-col hidden gap-2 text-xl font-bold md:flex md:pl-20 md:text-3xl text-neutral-300">
                  {item.image && <img src={item.image} className="w-[50px] mb-2 rounded-3xl"/>}
                  <h3 >{item.title}</h3>
            ...
            <div className="relative w-full pl-20 pr-4 md:pl-4">
              <div className="block mb-4 text-2xl font-bold text-left text-neutral-300 md:hidden ">
                {item.image && <img src={item.image} className="w-[50px] rounded-3xl"/>}
                  <h3 >{item.title}</h3>
                  <h3 className="text-2xl">{item.job}</h3>
                  <h3 className="text-2xl text-neutral-500">{item.date}</h3>
              </div>
